...
public class GameManager : MonoBehaviour
{
    ...
    public MetaLayer metaLayer;
    public EvilAI evilAI;
    public MLPlayer mlPlayer;
    public PlayerScript playerVictim { get; set; }
    ...
    public void Awake()
    {
        ...
        metaLayer = new MetaLayer(gameState);
        evilAI = new EvilAI(gameState);
        mlPlayer = new MLPlayer(gameState, metaLayer);
        ...
        gameState.pointsOfRouteLength = pointsOfRouteLength;
    ...
    void Start()
    {
        ...
        foreach (PlayerScript p in players)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                // add traincard to gameState cardDeck
                gameState.DrawCard(drawnCard.color);
        ...
        // time between moves of AI, 2 sec
        turnDelayAI = 2f;

        logAIactions = true;

        //Added to start the QTable in the MLPlayer
    ...
    void Update()
    {
        ...
        if (traincards.Count == 0 && traincardsDiscard.Count > 0)
        {
            ...
            {
                traincards.Add(card);
                gameState.RefillDeck(card.color);
            }
            //gameState.LogCardDeck();
    ...
    IEnumerator DestinationCardsSelectionAtStartProcess()
    {
        ...
        if (activePlayer.isEvilAI)
        {
            StartCoroutine(StartRuleProcessEvilAI());
        }

        if (activePlayer.playerName == "Dummy" && activePlayer.isMLAI)
        {
            StartCoroutine(StartRuleProcessDummy());
        }
        else if (activePlayer.isMLAI)
        {
            StartCoroutine(StartRuleProcessMLAI());
        }
    ...
    IEnumerator StartRuleProcessDummy()
    {
        Debug.Log(activePlayer.playerName + " am Zug in Runde: " + gameState.TurnCounter);
        DrawTrainCardFromDeck();
    ...
    IEnumerator StartRuleProcessMLAI()
    {
        ...
        if (turnCounter <= 2) // route weights are sometimes not created before turn 2
        {
            activePlayer.qTable.qDictionary = gameState.copyCityMap();
            yield return new WaitForSecondsRealtime(1);
        }
        ...
        //mlPlayer.qTable.PrintQTable();

        bool destCardFulfilled = true;
        
        // start of Q-learning for all destinationcards
        foreach (var destCard in activePlayer.handdestinationcards)
        {
            if (!mlPlayer.checkConnection(destCard.citiesAndPoints[0], destCard.citiesAndPoints[1]) && !destCard.fulfilled)
            {
                ...
                mlPlayer.startTraining(destCard.citiesAndPoints[0], destCard.citiesAndPoints[1], 1000, 0.3, 0.95, 100); // comment this for test purpose
                yield return new WaitForSecondsRealtime(1); // Warten auf Unity
            }
        }

        // Test1 Vancouver to Denver
        //mlPlayer.startTraining("Vancouver", "Denver", 1000, 0.2, 0.9, 100);

        // Experiment 2 LosAngeles to Miami
        //mlPlayer.startTraining("LosAngeles", "Miami", 1000, 0.3, 0.95, 100);
        //if (mlPlayer.checkConnection("LosAngeles", "Miami"))
        //    gameState.LogMoves();

        // Test4 El Paso to Denver
        //mlPlayer.startTraining("ElPaso", "Denver", 1000, 0.2, 0.95, 100);
        //yield return new WaitForSecondsRealtime(1); // Warten auf Unity
        ...
        // Debug for bestWay values
        //foreach (var x in mlPlayer.bestWay)
        //    Debug.Log("name: " + x.routeName +"weight: " + x.weight);
        ...
        //}
        //Debug.Log("Sum Weight: " + summe);
        //Debug.Log("Unique Routes = " + mlPlayer.getUniqueBestRoutes().Count());

        //mlPlayer.qTable.PrintQTableGrid();

        // Debug End ------------------------------------------------------------------------------

        // save the connection, which fulfilles the latest destinationcard
        activePlayer.bestWay = mlPlayer.getShortWay(); // select the shortest way
        //activePlayer.bestWay = mlPlayer.getBestWay(); // select the best way
        yield return new WaitForSecondsRealtime(1); // Warten auf Unity
            ...
            while (actionPoints > 0)
            {
                switch (bestAction)
                {
                    ...
                    case "pickTrainCardfromOpenCards":
                        ...
                        if (!cardDrawn && Array.Exists(faceupTraincards, card => card.color == "Joker") && actionPoints == 2)
                        {
                            AIdrawsOpenCard("Joker");
                            firstCardDrawn = true;
                        ...
                        else if (!cardDrawn && actionPoints > 0)
                        {
                            DrawTrainCardFromDeck();
                            Debug.Log(activePlayer.playerName + " picked Card from Deck");
                            yield return new WaitForSecondsRealtime(1);
                            actionPoints -= 1;
                        }
                        break;
                    ...
                    case "pickNewDestinationCard":
                        ...
                        yield return new WaitForSecondsRealtime(1);
                        int destinationCardPosition = ElectDestinationCardsInGame();
                        gameState.AddMove(activePlayer, "Pick new destinationcard", "", "");
                        StartCoroutine(AIselectsDestinationCard(destinationCardPosition));
                        break;
                }
                yield return new WaitForSecondsRealtime(1);
    ...
    {
        //yield return new WaitForSecondsRealtime(1); // Wait for Unity
        
        // Set this true, to use the blocking option
        bool toggleBuildRoutes = true;

        // Example Debug Logs
        Debug.Log("Active Player = " + gameState.Player.playerName);
        //foreach (var x in activePlayer.cities)
        //    Debug.Log("City = " + x);
        //foreach (var x in activePlayer.acquiredRoutes)
        //    Debug.Log("Route = " + x);
        //Debug.Log("Active Player = " + gameState.Player.playerName);
        //gameState.LogCardDeck();
        //Debug.Log("The next traincard from deck has most likely color: " + metaLayer.GetMostProbableCardToDraw());
        //gameState.LogDestinationCardsOfPlayers();
        //gameState.LogOpenCards();
        //gameState.LogDrawnCards();
        //gameState.LogMovesForPlayer(players[0].playerName);
        //Debug.Log("Total Cards in Deck = " + gameState.GetTotalCardsInDeck());
        //Debug.Log("Joker Cards in Deck = " + gameState.GetTotalCardsInDeckByColor("Joker")); 


        // **Variables**

        string targetRoute = "";
        // set the focus on a player
        PlayerScript victim = evilAI.PickVictim(); // or set a Player like "players[0]"
        
        // set victim back to type PlayerScript
        foreach (var p in players)
            if (p.playerName == victim.playerName)
                playerVictim = p;

        bool isJoker = false;
        string[] openCards = new string[5];
        firstCardDrawn = false;
        if (toggleBuildRoutes)
        {
            // Build Route: With Cheat: EvilAI steals a route from the victim destination cards, therefore:
            // 1: check for enemy DestinationCards
            //List<DestinationCard> allPlayersDestinationCards = gameState.DestinationCardsOfPlayers.Keys.ToList();
            List<DestinationCard> victimDestinationCards = evilAI.GetDestinationCardsOfVictim(victim.playerName);

            // 2: get routes from enemy DestinationCards
            Dictionary<string, int> edgesOfAllPaths = new Dictionary<string, int>();
            List<string> acquiredRoutes = playerVictim.acquiredRoutes;
            CreateWeights();
            yield return new WaitForSecondsRealtime(1); // Wait for Unity
            foreach (DestinationCard dstntncrd in victimDestinationCards)
            {
                ////yield return new WaitForSecondsRealtime(1); // Wait for Unity
                //Debug.Log("Der Spieler " + activePlayer.playerName + " hat Karten = " + playerVictim.handtraincards.Count());
                ////yield return new WaitForSecondsRealtime(1); // Wait for Unity

                ShortestPathFinder shrtstPthFndr = new ShortestPathFinder(cityMap);
                //find the shortest path from city1 (citiesAndPoints[0]) to city2 (citiesAndPoints[1]) and get back the predecessors
                Dictionary<string, string> predecessorsList = shrtstPthFndr.DijkstraReduceWeightsOfOwnRoutesAndReturnsPredecessors(dstntncrd.citiesAndPoints[0], playerVictim);
                //yield return new WaitForSecondsRealtime(1); // Wait for Unity
                //determine the edges of the shortest way from city1 to city2
                List<string> edges = GetPathEdges(dstntncrd.citiesAndPoints[0], dstntncrd.citiesAndPoints[1], playerVictim, predecessorsList);
                ...
                ShortestPathFinder shrtstPthFndr1 = new ShortestPathFinder(cityMap);
                Dictionary<string, string> predecessorsList1 = shrtstPthFndr1.DijkstraReduceWeightsOfOwnRoutesAndConsideredHandcardsAndReturnsPredecessors(dstntncrd.citiesAndPoints[0], playerVictim);
                //yield return new WaitForSecondsRealtime(1); // Wait for Unity
                //determine the edges of the shortest way from city1 to city2 by reducing the costs with the cards on hands
                List<string> edges1 = GetPathEdges(dstntncrd.citiesAndPoints[0], dstntncrd.citiesAndPoints[1], playerVictim, predecessorsList1);
                // add all elements from edges and edges1 to edgesOfAllPaths:
                foreach (string edge in edges)
                {
                    if (!edgesOfAllPaths.ContainsKey(edge))
                        edgesOfAllPaths.Add(edge, dstntncrd.points);
                }
            ...
            string[] edgesOfAllPathsArray = edgesOfAllPaths.Keys.ToArray();

            // 3: select a route that can be afforded
            yield return new WaitForSecondsRealtime(1); // Wait for Unity
            targetRoute = evilAI.PickRoute(edgesOfAllPathsArray, activePlayer, routes);
            yield return new WaitForSecondsRealtime(2); // Wait for Unity
            // 4: get that route
            if (!firstCardDrawn && !(targetRoute == ""))
            {
                StartCoroutine(AIbuysRoute(targetRoute));
                yield return new WaitForSecondsRealtime(1); // Wait for Unity
                firstCardDrawn = true;
        ...
        if (!firstCardDrawn)
        {
            for (int i = 0; i < 2; i++)
            {
                if (!isJoker)
                {
                    ...
                    var color = evilAI.GetVictimTrainCardColor(victim.playerName);
                    yield return new WaitForSecondsRealtime(1); // Warten auf Unity
                    if (openCards.Contains(color))
                    {
                        AIdrawsOpenCard(color);
                        if (color == "Joker") isJoker = true;
                        //Debug.Log("Draw from open Cards: " + color);
                        firstCardDrawn = true;
                    }
                    else
                    {
                        DrawTrainCardFromDeck();
                        //Debug.Log("Draw from deck");
                        firstCardDrawn = true;
                    }
                }
            }
    ...
    IEnumerator StartRuleProcessAI1()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            if (lastPlayerBeforeGameEnds != null)
            {
                if (!firstCardDrawn)
                {
                    // Streckenlängen von 1 bis 6
                    int[] routeLengths = { 6, 5, 4, 3, 2, 1 };

                    foreach (int routeLength in routeLengths)
                    {
                        // Überprüfe, ob die aktuelle Farbe oder "Grey" in der Liste der Strecken enthalten ist
                        if ((routes6.Contains(firstColor) || routes6.Contains("Grey")) && handcardsColorsCount[firstColor] + jokerCount >= routeLength && activePlayer.availableWagons >= routeLength)
                        {
                            // Strecken erwerben
                            for (int r = 0; r < routes.transform.childCount; r++)
                            {
                                RouteScript route = routes.transform.GetChild(r).GetComponent<RouteScript>();
                                if (route.owner == null && route.routeLength == routeLength && (route.routeColor == firstColor || route.routeColor == "Grey"))
                                {
                            ...
                            break; // Verlasse die Schleife nach dem ersten Treffer
                        }
                    }
                }
            }
            ...
            else if (!firstCardDrawn && (routes6.Contains(firstColor) || routes6.Contains("Grey")) && handcardsColorsCount[firstColor] + jokerCount >= 6 && activePlayer.availableWagons >= 6)
            {
                // strecke erwerben
                for (int r = 0; r < routes.transform.childCount; r++)
                {
    ...
    IEnumerator StartRuleProcessAI2()
    {
        ...
        else
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                {
                    Debug.Log("Failed to switch to AI#1");
                }
    ...
    IEnumerator AIbuysRoute(string routeName)
    {
        ...
        if (counter == routeLength)
        {
            gameState.AddMove(activePlayer, "Buy route", "", routeName);
    ...
    IEnumerator NewTurn()
    {
        if (!newTurnProcessing)
        {
            ...
            if (activePlayer.playerName.Contains("Dummy") && activePlayer.isMLAI)
            {
                yield return new WaitForSecondsRealtime(0.5F);
                StartCoroutine(StartRuleProcessDummy());
                yield return new WaitForSecondsRealtime(0.5F);
    ...
    public void OpenTrainCardFromDeck(int index)
    {
        ...
        gameState.DrawCard(faceupTraincards[index].color);
        gameState.OpenCards.Add(faceupTraincards[index].color);
        // add tag "ToBlock"
    ...
    public void DrawTrainCardFromDeck()
    {
        ...
        traincards.Remove(drawnCard);
        gameState.DrawCard(drawnCard.color);
        gameState.AddMove(activePlayer, "Draw card from deck", drawnCard.color, "");
        // drawn card to player's hand
    ...
    public void DrawTrainCardFromOpenCards(FaceUpTrainCardScript card)
    {
        ...
        activePlayer.AddTrainCardToHand(drawnCard);
        gameState.OpenCards.Remove(drawnCard.color);
        // create and display this card
        Instantiate(trainCardOnHandPrefab, handcardsPanel.transform.position, Quaternion.identity, handcardsPanel.transform);
        // give the handcard the sprite from the drawn card
        ...
        gameState.AddMove(activePlayer, "Draw card from open cards", drawnCard.color, "");
    ...
    public void OnClickConfirmDestinationCards()
    {
        for (int i = 0; i < destinationcardsDrawDisplay.transform.childCount; i++)
        {
            if (destinationcardsDrawDisplay.transform.GetChild(i).GetComponent<CardSelectScript>().getSelected())
            {
                ...
                gameState.DestinationCardsOfPlayers.Add(drawnCards[i], activePlayer.playerName);
    ...
    public IEnumerator StartTrainCardSelection(int routeLength, string routeColor, Transform routeParent)
    {
        ...
        // check, if player has clicked on confirmButton -> if highlighted is false, routeSelection was cancled
    ...
    */
    public bool CheckHandCardsForClaimingOfRouteWithoutJoker(int routeLength, string routeColor)
    {
        ...
        // if the route is gray, check whether the handcards contains any routeColor with enough cards to chose the selected route
         if (routeColor == "Grey") 
        {
    ...
    */
    ...
    public bool CheckEndingConditionOfGame()
    {
        ...
        {
            Debug.Log("Every player has one move left.");    
    ...
    */
    ...
    public void CreateCityMap()
    {
        ...
        cityMap.Add("Atlanta", new List<CityConnection> { new CityConnection("Nashville", "Nashville_Atlanta_Grey"), new CityConnection("Raleigh", "Raleigh_Atlanta_Grey_1"), new CityConnection("Raleigh", "Raleigh_Atlanta_Grey_2"), new CityConnection("Charleston", "Atlanta_Charleston_Grey"), new CityConnection("Miami", "Atlanta_Miami_Blue"), new CityConnection("NewOrleans", "Atlanta_NewOrleans_Yellow"), new CityConnection("NewOrleans", "Atlanta_NewOrleans_Orange") });
    ...
    public bool CheckCityConnection(string city1, string city2, PlayerScript player)
    {
        CreateWeights();
        ShortestPathFinder shortestPathFinder = new ShortestPathFinder(cityMap);
        Dictionary<string, int> shortestPathList = new Dictionary<string, int>();
        shortestPathList = shortestPathFinder.DijkstraReduceWeightsOfOwnRoutes(city1, player);
        
        // Added for MLPlayer
        //if (player.isMLAI)
        //{
        //    shortestPathList = mlPlayer.getCityConnectionWeights(mlPlayer.getCityConnection(city1, city2, player, 100));
        //}
        //else
        //{
        //    shortestPathList = shortestPathFinder.DijkstraReduceWeightsOfOwnRoutes(city1, player);
        //}
        // End of MLPlayer extension
    ...
    public List<string> GetPathEdges(string city1, string city2, PlayerScript player, Dictionary<string, string> predecessorsList)
    {
        ...
        while(currentCity != city1)
        {
            ...
            {
                Debug.Log("Predecessor from " + currentCity + " is null!");
                return edges;
            }
